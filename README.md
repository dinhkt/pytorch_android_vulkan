...
This work follows: https://pytorch.org/tutorials/prototype/vulkan_workflow.html
...
You can use the provided models in app/src/main/assets, but if you want to generate the model by yourself, refer to the convert script for each model in script_model folder.
...
The release torch version installed by pip will not work when generating vulkan-supported models and/or using the models. We need to build torch from source with vulkan enable and then use that torch to generate the models.
...
```
It seems like a limit of vulkan when not supporting tensor that has more than 4 dimensions. 

Let's wait for the future pytorch update!
