...
def export_torchscript(model, im, file, optimize,vulkan, prefix=colorstr('TorchScript:')):
    # YOLOv5 TorchScript model export
    try:
        LOGGER.info(f'\n{prefix} starting export with torch {torch.__version__}...')
        f = file.with_suffix('.torchscript')
        fl = file.with_suffix('.torchscript.ptl')
        f_vulkan=file.with_suffix('.vulkan.pt')
        print(im.shape)
        ...
        if optimize:  # https://pytorch.org/tutorials/recipes/mobile_interpreter.html
            ...
            if vulkan:
                ...
                ts_opt=optimize_for_mobile(ts,backend='vulkan')
                torch.jit.save(ts_opt,str(f_vulkan))
            else:
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    if 'torchscript' in include:
        export_torchscript(model, im, file, optimize,vulkan)
...
def parse_opt():
    ...
    parser.add_argument('--vulkan', action='store_true', help='Vulkan support')
